...
簡易頁面會顯示原文、資訊取得方式、查核狀態、目前流程狀態與安全提醒；尚未保存時會標示尚未交出草稿。
...
整理者按下「保存並交出草稿」後，回到簡易頁面，顯示本頁保存的交接摘要。
交出後仍要標示這不是已確認資料，也不是任務或行動建議。
...
- 保存並交出草稿時，要記錄這只是本頁狀態，不代表正式提交或查核完成。
...
- 修正後：流程改成「簡易頁 → 編輯草稿 → 保存並交出 → 簡易交接頁」，並保留三種手動流程輸出。
...
- 左側列表只顯示「已交出」是否足夠，還是需要另外顯示誰交出、何時交出。
